...
import { 
  ...
  CheckCircle2, AlertCircle, Trash2, Wallet, ArrowDownLeft, ArrowUpRight, Phone, UserPlus
} from 'lucide-react';
...
export default function Debts() {
  ...
  const fetchDebts = async () => {
    try {
      ...
      console.log('Debts API response:', res.data); // Debug uchun
      ...
      console.log('Parsed debts data:', debtsData.length, 'items'); // Debug uchun
      setDebts(debtsData);
    } catch (err) { 
  ...
  const fetchCustomers = async () => {
    try {
      ...
      console.log('Customers API response:', res.data); // Debug uchun
      ...
      console.log('Parsed customers data:', customersData.length, 'items'); // Debug uchun
      ...
      <div className="p-1 sm:p-2 space-y-2 sm:space-y-3 w-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          {statItems.map((stat, i) => (
            ...
            >
              {/* Pro Card Design */}
              <div className={`relative bg-white rounded-2xl p-5 shadow-sm hover:shadow-xl transition-all duration-500 ${stat.filter ? 'hover:-translate-y-1' : ''} overflow-hidden border border-slate-100`}>
                {/* Top Gradient Border */}
                <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${
                  stat.color === 'success' ? 'from-emerald-400 to-emerald-500' :
                  stat.color === 'danger' ? 'from-red-400 to-red-500' :
                  stat.color === 'accent' ? 'from-purple-400 to-purple-500' :
                ...
                <div className={`absolute -right-6 -bottom-6 w-24 h-24 rounded-full opacity-20 group-hover:scale-150 transition-transform duration-500 ${
                  stat.color === 'success' ? 'bg-emerald-50' :
                  stat.color === 'danger' ? 'bg-red-50' :
                  stat.color === 'accent' ? 'bg-purple-50' :
                  'bg-blue-50'
