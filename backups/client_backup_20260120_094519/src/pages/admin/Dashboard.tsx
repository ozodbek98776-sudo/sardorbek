...
import { 
  DollarSign, TrendingUp, ShoppingCart, Clock, RefreshCw, ArrowUpRight, ArrowDownRight, BarChart3, Activity, Package
} from 'lucide-react';
      ...
      <div className="p-3 lg:p-6 space-y-6 pb-20 lg:pb-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-1">
            <h2 className="text-2xl lg:text-3xl font-bold bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">
              {t('dashboard.overview')}
            </h2>
            <p className="text-slate-600">{t('dashboard.description')}</p>
          </div>
          ...
          <>
            ...
            <div className="grid 2xs:grid-cols-1 xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
              {mainStats.map((stat, i) => (
                <div key={i} className="group relative bg-white/80 backdrop-blur-sm rounded-lg sm:rounded-xl md:rounded-2xl p-3 sm:p-4 md:p-6 shadow-md border border-slate-200/50 hover:shadow-lg hover:border-slate-300/50 transition-all duration-300 hover:-translate-y-1">
                  {/* Background Gradient */}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Tooltip 
                      ...
                      formatter={(value: number) => [`${formatNumber(value)} UZS`, 'Daromad']}
                    />
                    <Area 
                      type="monotone" 
                      dataKey="sales" 
