...
export default function DebtApprovals() {
  ...
  const handleApprove = async (debtId: string) => {
    const confirmed = await showConfirm(
      "Bu qarzni tasdiqlaysizmi? Tasdiqlangandan keyin mijozning umumiy qarziga qo'shiladi.",
      "Tasdiqlash"
    );
    if (!confirmed) return;
  ...
  const handleReject = async (debtId: string) => {
    ...
    const confirmed = await showConfirm(
      "Bu qarzni rad etasizmi? Rad etilgan qarz o'chiriladi.",
      "Rad etish"
    );
    if (!confirmed) return;

    try {
      await api.post(`/debts/${debtId}/reject`, { reason });
      await fetchPendingDebts();
